...
import { buildDatastoreDevice, readDatastore } from './src/datastores.js';
...
async function discover() {
  ...
  const devices = stores.map((store) => buildDatastoreDevice(gladys, store, config));
...
gladys.onPoll(async (device) => {
  ...
  if (!store) return;
  await gladys.publishStates(await readDatastore(gladys, store, config));
});
